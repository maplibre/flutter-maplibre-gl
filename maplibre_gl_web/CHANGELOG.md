...
* A map that comes up without a renderer now logs why, once per session. Version 6 requires WebGL2 and dropped the WebGL1 fallback, and rather than throwing like version 5 it fires an `error` from inside the constructor, too early to subscribe to, so the map would otherwise just be blank. The message tells a WebGL1 only browser apart from one with no WebGL at all, and names the version 5 build as the way out of the first (#943).
