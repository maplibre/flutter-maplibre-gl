...
* The manual location puck is drawn by the plugin, since maplibre-gl-js has no injectable location component. It reuses maplibre-gl-js' own `maplibregl-user-location-dot` and `maplibregl-user-location-accuracy-circle` classes for a native look, plus a bearing arrow, relying on `maplibre-gl.css`, which the plugin loads together with the library (#840).
* The plugin loads maplibre-gl-js itself before the first map is built, so apps no longer add the script and stylesheet tags to `web/index.html` and should remove them. By default it injects the exact build its interop is written against (`kMapLibreJsVersion`), or whatever `MapLibreMap.webLibrarySource` is configured to; an existing `maplibregl` global is reused as it is, so a manually pinned copy silently overrides the version the package expects. A failed stylesheet only logs, since it affects how the controls and the puck look and not the map itself, and a failed load clears the memoized attempt so the next map build retries (#928).
* `MapLibreGlobalWeb` implements the new `MapLibreGlobalPlatform` and is installed at plugin registration, giving `preWarm()` and `ensureWebLibraryLoaded()` their web behavior; the `maplibregl.prewarm` interop moved here from `maplibre_gl`, which no longer carries JS interop (#928).
