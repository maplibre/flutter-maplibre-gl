...
* **Android, iOS**: `fillExtrusionRoundedCornerDistance` rounds the corners of extruded shapes. MapLibre GL JS does not implement it, so web ignores it (#957).
...
* **Android, iOS**: the two offline download errors that carried a vendor name in their code are renamed: `mapboxTileCountLimitExceeded` is now `tileCountLimitExceeded` and `mapboxInvalidRegionDefinition` is now `invalidRegionDefinition`. Only the `PlatformException` code changes; when and why they fire does not.
...
* **Web**: MapLibre GL JS upgraded from 5 to 6; see Actions needed above (#943).
...
* **Example app**: the Data-Driven Expressions demo drew the USA as a triangle across the Atlantic; one coordinate was missing its minus sign.
