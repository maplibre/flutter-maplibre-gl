...
* **Web**: properties MapLibre GL JS supports but MapLibre Native does not implement yet are exposed too: `iconOverlap` and `textOverlap` on symbols, `fillLayerOpacity` on fills, `lineLayerOpacity` on lines, `resampling` on raster and hillshade layers, `filter` and `clusterMinPoints` on GeoJSON sources, the `custom` raster-dem encoding with its `redFactor`, `greenFactor`, `blueFactor` and `baseShift` factors, and the `Expressions.globalState`, `join` and `split` expressions. Android and iOS ignore them (#957).
...
* `volatile` shows up on vector, raster and raster-dem sources because the style spec defines it, but nothing reads it yet: MapLibre GL JS will not implement it and the plugin does not forward it to the native SDKs, so setting it has no effect for now (#957).
