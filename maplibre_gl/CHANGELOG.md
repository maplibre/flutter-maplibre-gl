...
* **Android, iOS, Web**: the style property surface is regenerated from the current MapLibre style spec instead of a four-year-old copy. New layer properties: `textVariableAnchorOffset` on symbols, `fillExtrusionRoundedCornerDistance` on fill-extrusions, `hillshadeMethod` and `hillshadeIlluminationAltitude` for multidirectional hillshading. New expressions: `Expressions.distance`, `within`, `indexOf`, `slice` and `elevation`. New source options: `volatile` on vector, raster and raster-dem sources, `clusterMinPoints` and `filter` on GeoJSON sources, and `redFactor`, `greenFactor`, `blueFactor`, `baseShift` plus the `custom` encoding on raster-dem sources for terrain tiles with a custom height encoding (#957).
...
* **Web**: properties MapLibre GL JS supports but MapLibre Native does not implement yet are exposed too: `iconOverlap` and `textOverlap` on symbols, `fillLayerOpacity`, `lineLayerOpacity` and `resampling` on raster and hillshade layers, and the `Expressions.globalState`, `join` and `split` expressions. Android and iOS ignore them (#957).
...
* **Android**: a hillshade layer takes its colours again. `hillshadeShadowColor`, `hillshadeHighlightColor` and `hillshadeIlluminationDirection` became arrays in MapLibre Native 6.24 for multidirectional hillshading, so a single value was rejected with `Expected array<color> but found string instead` and the layer silently kept the default black and white. A single value is now wrapped for you, and a list of values is passed through, matching what iOS already did (#957).
...
### Changed
* The style spec the code is generated from (`scripts/input/style.json`) is now a verbatim copy of a pinned upstream release (`@maplibre/maplibre-gl-style-spec`, currently 26.2.1). `melos run check-style-spec` fails on any local drift and reports what a newer release would bring to each platform, a weekly workflow opens the update PR, and the generator reads the spec's `sdk-support` metadata against the pinned MapLibre Native versions, so properties the native SDKs do not implement yet are generated for web only and upgrading the SDKs unlocks them automatically (#957).
