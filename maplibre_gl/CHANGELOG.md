...
* **Android, iOS**: `MapLibreMap.preWarm()` no longer throws "Binding has not yet been initialized" when called at the top of `main()` (#867).
...
* **Annotations**: the section overview now states the two conditions every annotation depends on, and Annotations vs Style Layers collects the constraints of both APIs in one place (#910).
* **Android**: `MapLibreMap.useHybridComposition` was documented with the wrong default; it has been `false` since 0.16.0. Its docs and the [performance page](https://maplibre.org/flutter-maplibre-gl/advanced/performance/#platform-view-mode-android) now describe what each value selects (#816).
