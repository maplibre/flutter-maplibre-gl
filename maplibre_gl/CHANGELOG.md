...
* **Android**: `setCustomHeaders` no longer writes header values to the log. An app that passed an API key or an `Authorization` header to its tile provider had that credential written to Logcat in plain text, in release builds too, where any process able to read the device log could pick it up. The log line now names the header without its value, and the web implementation, which printed the same values to the browser console, does the same (#1015).
...
* **iOS**: fixed random `EXC_BAD_ACCESS` crashes while a map was on screen, introduced in 0.26.2 together with custom header support. Every tile request got its own `URLSession`, and a tile that was cancelled at the same moment it completed had its task released twice from two threads at once, which crashed inside CFNetwork. All intercepted requests now share one session and the teardown is synchronized, so a cancellation that races the response is handled safely. Apps that never set custom headers no longer route their requests through the header injection at all. Two further problems on that path are fixed with it: a redirected tile or style was fetched twice, once by the restarted load and once by the redirect being followed as well, and `setOfflineMaxConcurrentRequests` had no effect on requests carrying custom headers, because those are loaded on a session of their own that the limit never reached (#1009).
...
* **Web**: `setGlobalStateProperty()` sets a value in the style's global state, which the `Expressions.globalState` expression reads, so one switch restyles any number of layers at once. Android and iOS throw an `UnsupportedError`. See the [expressions guide](https://maplibre.org/flutter-maplibre-gl/advanced/expressions/) (#960).
