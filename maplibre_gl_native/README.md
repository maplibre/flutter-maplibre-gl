...
The MapLibre runtime and maps live on a **dedicated engine isolate**: every command, query, and event crossing the boundary is isolate-sendable, and heavy tile-integration work never stalls the UI thread. (Benchmarks of the retired single-isolate mode against this architecture are in `../maplibre_gl_example/docs/benchmarks/ffi-benchmark-results-2026-07.md`; the isolate won everywhere, so it is now the only mode.) A `gettid` watchdog rebinds the runtime if the VM ever migrates the isolate across OS threads (the native handles are thread-affine; see `upstream_patches/`), and failure is loud rather than silent: a failed bootstrap throws, a command that fails inside the engine comes back as an `EngineErrorEvent`, and if the isolate itself ever dies the host fails every pending query, emits a terminal `EngineDiedEvent`, and makes later calls throw instead of hanging (every query also carries a 15-second deadline as a backstop). The spawn is shared: two maps mounted in the same frame get one engine, not two racing onto one tile cache.

**Drawing is not on that isolate.** The shim thread that owns the AChoreographer also owns the live render session and calls `render_update` itself, so no Dart work (GC, HTTP, a style parse) can delay a display frame. A render session's owner thread is checked on every entry point and Dart still needs several of them (feature queries, feature state, resize, surface replace, detach), so ownership ping-pongs under one mutex: those calls borrow the session back for the length of one call while renders, which are 60-90 times more frequent, simply keep it. One thread draws one session: with two maps live, the most recent surface attach holds the display thread while the other map draws on the engine isolate, and teardown withdraws only the departing map's session (re-offering the survivor), so disposing one map cannot blank the other. Dart still pumps the runtime, still on the display pulse, because a resize reaches the map only through its owner thread and all tile and style work lands there.
...
- Events: style-loaded, style-load-failure (`onMapLoadingErrorPlatform` on the platform interface), camera move/idle, map idle, user-location, camera-tracking changed/dismissed, feature tapped/dragged, plus the engine-health events above (`EngineErrorEvent`, `EngineDiedEvent`).
...
- Ornaments: compass (auto-hidden when north-up, tap resets north), attribution (collapsible pill with per-source strings from the style and tappable links), the official MapLibre wordmark, an opt-in metric scale bar (`MapLibreGlNative.scaleBarEnabled = true`), all four corner positions plus margins.
- HTTP: the native client by default, the Dart provider for regex-filtered headers (see above); per-map `setCustomHeaders` and process-global `MapLibreGlNative.setGlobalHttpHeaders` are independent scopes, merged on every request with the per-map value winning a name collision.
