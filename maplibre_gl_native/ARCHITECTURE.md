...
flowchart TD
    ...
    subgraph ENGINE["Engine isolate (the map's world)"]
        ...
        CORE["EngineCore<br/><small>src/engine/core/engine_core.dart<br/>owns every native handle</small>"]
    end
...
**Moving.** Something changed what the map should look like (your finger, a camera animation, a tile that just arrived), so the engine draws, **exactly once per screen refresh**: 60 times a second on a 60 Hz screen, 120 on a 120 Hz one, and never more than that. Once about half a second goes by with nothing new to show, or the screen turns off, it goes back to still.
...
So instead of guessing the rhythm, we listen to it. Android publishes it through a system service (`Choreographer`) that calls you back right after every refresh. A small piece of native code (`cpp/shim.c`) subscribes to those callbacks and forwards each one to the engine isolate as a single message: that message is what this codebase calls the **vsync pulse** ("vsync" is the display's own refresh signal). Every pulse means: draw one frame, now, with a full refresh interval of headroom before the next deadline.
...
MapLibre Native's objects are **thread-affine**: each one belongs to the thread it was created on and may only be used from that thread. Using one from elsewhere does not raise an error, it quietly corrupts state and crashes later somewhere unrelated. That single fact is why the package is shaped the way it is: one isolate owns every native handle, and nothing else is allowed near them.
...
- The Dart VM may move an isolate onto a different OS thread between two callbacks, which would silently break that ownership. A check before every native call notices the thread changed and tells the native runtime to adopt the new one (`mln_runtime_rebind_thread`, one of our upstream patches). A "runtime rebound" line in the log is that check doing its job, not a warning.
- The vsync pulse comes in on a native thread that deliberately never calls MapLibre Native: it only posts a timestamp to the engine isolate's port, and the drawing happens there, on the owning thread.
...
| `src/engine/core/` | the engine isolate itself: `EngineCore` + parts, frame driver, vsync, HTTP provider, JSON converters | you add engine capabilities or change how frames render |
