...
* `MapLibreMap.preWarm()` starts up the MapLibre engine before the first map widget is built. Call it from `main()` to move native start-up work off the first map display, saving roughly 170 to 480 ms on Android, 45 to 165 ms on iOS and 10 to 50 ms on web (#867).
