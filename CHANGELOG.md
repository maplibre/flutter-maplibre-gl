...
* `MapLibreMap.preWarm()` starts up the MapLibre engine before the first map widget is built, for apps whose first screen is a map. Call it from `main()` to move native start-up work off the first map display, saving roughly 170 to 480 ms on Android, 45 to 165 ms on iOS and 10 to 50 ms on web (#867).
...
* **Android, iOS, Web**: an app can drive the user-location puck from its own updates instead of the device GPS, and no location permission is required in that mode. Build the map with `locationSource: ManualLocationSource()` and `myLocationEnabled: true`, then push each fix with `controller.updateManualLocation(ManualLocationUpdate(target: ..., bearing: ..., speed: ..., horizontalAccuracy: ...))`. Useful for a paired GPS device, a replayed track, or a simulation. The accuracy ring, the tracking modes and `onUserLocationUpdated` keep working, and the default stays `PlatformLocationSource()`. On web the plugin draws the puck itself, reusing the user-location styles from `maplibre-gl.css`, which the plugin loads together with the library (#840).
* **Web**: the plugin now loads MapLibre GL JS itself, so the `<script>` and `<link>` tags in `web/index.html` are no longer needed and apps should remove them. An existing `maplibregl` global is reused as it is, which means a manually pinned copy silently overrides the version the plugin is tested against. Where the library comes from is configurable through `MapLibreMap.webLibrarySource`: the build the plugin is tested against (the default), a self-hosted copy via `MapLibreJsSource.urls` for apps whose Content-Security-Policy rules out a CDN, or `MapLibreJsSource.preloaded` when the page loads the library itself. `MapLibreMap.preWarm()` starts the download during app start-up, and the new `MapLibreMap.ensureWebLibraryLoaded()` completes once `maplibregl` is usable, for apps that call into MapLibre GL JS with their own JS interop, for example to register a protocol with `addProtocol` (#928).
* **Android**: the feature state API (`setFeatureState`, `getFeatureState` and `removeFeatureState` on `MapLibreMapController`) now works on Android as well as web, enabled by the MapLibre Android SDK 13.4.1 upgrade below. Feature state restyles individual features without touching the source data, so recolouring many features, which previously required re-feeding the whole GeoJSON source every frame, becomes a cheap per-feature call. On iOS the calls now fail with an error saying feature state is unavailable there because the MapLibre iOS SDK does not expose the API yet, instead of wrongly claiming Android is unimplemented too. `promoteId` remains web-only, so on Android features must carry a top-level `id` in the GeoJSON itself (#889).

### Fixed
* **Android, iOS**: symbols added through `addSymbol` are visible again on styles whose glyph server does not host `Open Sans Regular,Arial Unicode MS Regular`, which was the old default text font. MapLibre Native requests a symbol layer's font stack as one comma-joined token, so that default only resolved on a few glyph servers, and a 404 hid the whole symbol, icon included. The default is now `Noto Sans Regular`, which those servers do host. The font applies to the whole annotation layer and is not settable per symbol; apps that need a different one can add a symbol style layer with `addSymbolLayer` and set `textFont` there. Web still defers to the active style (#940).
...
* **Android, iOS**: `MapLibreMap.preWarm()` no longer throws "Binding has not yet been initialized" when called where its own documentation puts it, as the first statement of `main()` before `runApp()`. It reaches native over a method channel, which needs the Flutter binding that `runApp()` would normally create, so it now creates the binding itself. The call is idempotent, so an app that already calls `WidgetsFlutterBinding.ensureInitialized()` is unaffected. A test or app that needs a different binding, such as `IntegrationTestWidgetsFlutterBinding` or a custom subclass, must initialize it before calling `preWarm()`, since the first binding created wins. The call is skipped on background isolates, where the channel does not use the binding (#867).
...
* **Example app**: a new Feature State page demonstrates multi-selection on tap, exercising `setFeatureState`, `getFeatureState` and every shape of `removeFeatureState` on web and Android. The Hover Effect example stays web only, since it follows the mouse pointer (#889).
...
### Internal
* **Web**: the global, before-any-map calls (`MapLibreMap.preWarm()`, and the new `MapLibreMap.ensureWebLibraryLoaded()`) reach the web implementation through the platform interface (`MapLibreGlobalPlatform`), which the web package replaces at registration. This removes the conditional-import pair that put JS interop in the main package (#928).
