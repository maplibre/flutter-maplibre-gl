...
### Internal
* **Web**: the global, before-any-map calls (`preWarm()`, and the new `ensureWebLibraryLoaded()`) reach the web implementation through the platform interface (`MapLibreGlobalPlatform`), which the web package replaces at registration. This removes the conditional-import pair that put JS interop in the main package (#928).
