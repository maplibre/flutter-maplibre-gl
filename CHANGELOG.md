...
* **Android, Web**: `queryRenderedFeaturesInRect()` applies the `filter` it is given. This call takes the filter as a JSON string, which neither platform decoded, so it never reached the query and the answer held every feature in the rectangle (#953).
* **Android**: `queryRenderedFeatures()` and `querySourceFeatures()` say what is wrong with a call they cannot answer, such as one made before the style has loaded. They read their arguments blind, so the failure arrived as a bare `error` carrying a `NullPointerException` message (#954).
...
### Docs
* New guides for [user location](https://maplibre.org/flutter-maplibre-gl/advanced/user-location/), [startup and performance](https://maplibre.org/flutter-maplibre-gl/advanced/performance/) and [feature state](https://maplibre.org/flutter-maplibre-gl/advanced/feature-state/); this release's smaller additions are documented on the camera, styles and markers pages (#840, #867, #889).
