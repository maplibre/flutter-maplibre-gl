...
* `MapLibreGlobalPlatform`, the platform interface for calls that are global to the plugin rather than tied to a single map, with `MapLibreGlobalMethodChannel` as the method-channel default. The web package replaces the instance when it registers, the same way it replaces `MapLibrePlatform.createInstance`, which gives `MapLibreMap.preWarm()` and `MapLibreMap.ensureWebLibraryLoaded()` a route to the web implementation without JS interop in the main package (#928).
* `MapLibreMethodChannel` forwards `setFeatureState`, `removeFeatureState` and `getFeatureState` over the method channel instead of throwing `UnimplementedError`, so feature state, which lets a recolour of many features skip re-feeding the whole GeoJSON source, now works on Android. On iOS the calls throw an `UnsupportedError` naming the platform, because the MapLibre iOS SDK does not expose the feature state API yet (#889).
